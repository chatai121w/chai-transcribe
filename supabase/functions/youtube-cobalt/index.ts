...
async function fetchWithFallbacks(url: string, opts: ReqBody) {
  ...
  // Innertube is deliberately not imported: youtubei.js caused Brotli startup
  // crashes in the Edge runtime, turning even metadata requests into HTTP 503.
  attempts.push('innertube:removed');

  // 1) Self-hosted Cobalt if configured
  if (SELFHOST) {
    try {
