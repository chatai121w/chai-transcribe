...
==========================================
לוחץ Ctrl+Shift+H (ניתן לשנות) מכל מקום ב-Windows →
אוברליי קטן מופיע עם מד עוצמה → מקליט → Whisper מתמלל →
מדביק אוטומטית חזרה לשדה שממנו לחצת.

הפעלה:
    .venv/Scripts/python.exe server/voice_hotkey.py
    .venv/Scripts/python.exe server/voice_hotkey.py --hotkey ctrl+shift+t --port 3000
"""
...
parser.add_argument('--hotkey', default='ctrl+shift+h')
...
# ── Shared state ─────────────────────────────────────────────────────────────
_recording    = threading.Event()   # thread-safe bool
_audio_frames: list = []
_stream       = None
_prev_hwnd    = None

# UI widgets — only touched from the main thread
_overlay_win  = None
_status_lbl   = None
_level_canvas = None
_level_bar    = None
...
# Single queue: all worker threads → main thread
...
def _audio_cb(indata, frames, time_info, status):
    if _recording.is_set():
        ...
        level = min(rms * 10.0, 1.0)   # scale: speech ~0.02-0.15 RMS → 0-1
...
def _worker_transcribe(frames: list, hwnd: int):
    ...
    time.sleep(0.5)
    _ui_queue.put(('CLOSE',))
    paste_text(text, hwnd)   # runs after overlay is closed

# ── Recording control ─────────────────────────────────────────────────────────
def _cmd_start():
    """Hotkey thread → main thread: begin recording."""
...
def _cmd_stop():
    """Stop stream, launch transcription worker (safe from any thread)."""
    global _stream
    _recording.clear()
    if _stream:
        _stream.stop()
...
def _create_overlay():
    ...
    # Background track
    _level_canvas.create_rectangle(0, 4, METER_W, 12,
                                    fill='#21262d', outline='')
    # Animated bar
    _level_bar = _level_canvas.create_rectangle(0, 4, 0, 12,
                                                 fill='#3fb950', outline='')
...
# ── Main-thread event poll (runs every 30 ms) ─────────────────────────────────
def _poll(root: tk.Tk):
    ...
    try:
        while True:
            ...
            if cmd[0] == 'SHOW':
                _create_overlay()
                # Open audio stream (callbacks are on sounddevice thread)
                _recording.set()
                ...
                _, text, color = cmd
                if _status_lbl:
                    _status_lbl.configure(text=text, fg=color)
                if _dot_lbl:
                    _dot_lbl.configure(fg=color)
                if _level_canvas and _level_bar:
                    # Freeze meter at 0 while processing
...
if __name__ == '__main__':
    ...
    root = tk.Tk()
    root.withdraw()           # invisible root keeps event loop alive
    root.after(30, lambda: _poll(root))
    root.mainloop()
