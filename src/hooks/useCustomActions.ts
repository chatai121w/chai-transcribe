 ...
 * Stored in localStorage AND synced to the cloud (Supabase) per user.
 * Built-in actions can be hidden but not deleted.
 */
...
export type ViewMode = 'grid' | 'compact' | 'list' | 'masonry' | 'columns2' | 'columns3' | 'columns4';
...
export function useCustomActions() {
  ...
  // ── Cloud: pull on mount (once user is signed in) ──
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        const { data, error } = await (supabase as any)
          .from('ai_editor_custom_actions')
          .select('payload, view_mode, updated_at')
          .eq('user_id', uid)
          .maybeSingle();
        ...
        const cloudActions = data.payload as CustomAction[] | null;
        if (Array.isArray(cloudActions) && cloudActions.length > 0) {
          // Merge built-ins for forward-compat
          const ids = new Set(cloudActions.map((a) => a.id));
          const missing = BUILTIN_ACTIONS.filter((b) => !ids.has(b.id));
          setActions([...cloudActions, ...missing]);
        }
        if (data.view_mode) setViewMode(data.view_mode as ViewMode);
        cloudLoaded.current = true;
      } catch {
        cloudLoaded.current = true;
      }
  ...
  // ── Cloud: debounced push on change ──
  useEffect(() => {
  ...
  const cycleViewMode = useCallback(() => {
    const order: ViewMode[] = ['grid', 'list', 'compact', 'masonry', 'columns2', 'columns3', 'columns4'];
